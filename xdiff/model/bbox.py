...
class BoundingBox:
    ...
    def __post_init__(self) -> None:
        if self.lon_min >= self.lon_max:
            raise ValueError(f"Bounding box longitude min ({self.lon_min}) must be less than max ({self.lon_max}).")
        if self.lat_min >= self.lat_max:
            raise ValueError(f"Bounding box latitude min ({self.lat_min}) must be less than max ({self.lat_max}).")
        if not (-90.0 <= self.lat_min <= 90.0 and -90.0 <= self.lat_max <= 90.0):
            raise ValueError(f"Bounding box latitude must be within [-90, 90], got {self.lat_min}..{self.lat_max}.")
